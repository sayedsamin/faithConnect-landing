  ...
  return (
    ...
    >
      <section className="rounded-md border border-border bg-background/86 p-6 shadow-[0_18px_34px_rgba(23,58,64,0.08)]">
        <p className="island-kicker">Learning Space</p>
        <h1 className="display-title mt-3 text-4xl font-bold text-foreground">
          Student dashboard
        </h1>
        <p className="mt-4 max-w-3xl text-base leading-7 text-muted-foreground">
          Keep track of your learning plan, upcoming tasks, and progress markers
          in a focused workspace.
        </p>

        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {[
            ['Current focus', 'Active', 'Learning plan ready'],
            ['Milestones', '3', 'Recently completed'],
            ['Next step', 'Review', 'Open tasks when ready'],
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-3">
          {[
            [BookOpenCheck, 'Continue learning plan'],
            [ChartNoAxesCombined, 'Review milestones'],
            [CalendarCheck, 'Prepare for sessions'],
          ].map(([Icon, label]) => (
            <button
              key={label as string}
              type="button"
