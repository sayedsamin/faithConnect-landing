...
import { CalendarPlus, Pencil, RefreshCcw, Search } from 'lucide-react'
...
export function SuperadminProgramsPanel({
  activeStatus,
}: {
      ...
      {visiblePrograms.length ? (
        ...
        <div className="mt-8 rounded-md border border-border p-6">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
            Programs will appear here as superadmins create and schedule them.
          </p>
        </div>
      )}
    </section>
    ...
    <tr className="border-t border-border align-top">
      ...
      <td className="px-4 py-4">
        <div className="flex flex-col gap-2 sm:flex-row">
          <Link
            to="/dashboard/superadmin/programs/$programId"
          ...
          <Link
            ...
            params={{ programId: program.id }}
            className="inline-flex min-h-9 items-center justify-center gap-2 rounded-md border border-primary bg-primary px-3 py-1.5 text-xs font-extrabold text-primary-foreground touch-manipulation hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/50"
            style={{ color: 'var(--white)' }}
          >
