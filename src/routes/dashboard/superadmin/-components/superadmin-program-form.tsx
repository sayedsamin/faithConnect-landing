...
import { useUpdateAdminProgramMutation } from '#/portals/admin/programs/mutations'
...
type SuperadminProgramFormProps = {
  programId: string
  initialValues: CreateAdminProgramFormValues
  programName: string
}

export function SuperadminProgramForm({
  programId,
  initialValues,
  programName,
}: SuperadminProgramFormProps) {
  ...
  async function handleUpdateProgram(values: CreateAdminProgramInput) {
    setStatusMessage(null)

    try {
      const payload: UpdateAdminProgramInput = {
        id: programId,
        ...values,
      }

      const updatedProgram = await updateProgramMutation.mutateAsync(payload)
      reset(values)
      setFocus('name')
      setStatusMessage(`${updatedProgram.name} was updated successfully.`)
    } catch (error) {
      setStatusMessage(getErrorMessage(error, 'Unable to update that program.'))
    }
  }
    ...
    <section className="rounded-md border border-border bg-background/86 p-6 shadow-[0_18px_34px_rgba(23,58,64,0.08)]">
      <div>
        ...
        <p className="mt-4 max-w-3xl text-base leading-7 text-muted-foreground">
          Update details for {programName}.
        </p>
      </div>
      ...
      <form
        ...
        noValidate
        onSubmit={(event) => void handleSubmit(handleUpdateProgram)(event)}
      >
